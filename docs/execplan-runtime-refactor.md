# Runtime/Engine Layering (Interventions removed)

This ExecPlan is a living document and must be maintained per `.agent/PLANS.md`.

## Purpose / Big Picture

Move `agtrace` from a CLI-centric watcher into a layered runtime where pure parsing lives in `agtrace-engine`, stateful orchestration and reactors live in a new `agtrace-runtime`, and `agtrace-cli` is only responsible for presentation. Users should be able to run `agtrace watch` and see the same stream output while the system now supports structured runtime events and testable parsing of state updates with no I/O.
...
- [x] (2025-12-17 18:05Z) Phase 3 (obsolete): Added runtime intervention plumbing and `RuntimeEvent::InterventionExecuted`; this feature was later removed as unused.
- [x] (2025-12-17 18:20Z) Phase 4: Removed `TuiRenderer` from reactors, moved stream rendering to `TraceView::render_stream_update` consuming `RuntimeEvent::StateUpdated`, and kept `watch` handler focused on runtime setup/subscription. Tests: `cargo test -p agtrace-cli handlers::watch`.
- [x] (2025-12-17 18:44Z) Interventions removed from runtime/CLI; RuntimeEvent/RuntimeConfig no longer include executor/intervention types.
...
- Decision: Shifted rendering to the `TraceView` observer by implementing event printing/token summaries in `render_stream_update` and removing `TuiRenderer` from the reactor set. Rationale: Aligns with the runtime observer model and keeps reactors pure decision-makers. Date/Author: 2025-12-17 / Assistant.
- Decision: Removed intervention surfaces from runtime and CLI as the feature is unused; runtime events now exclude executor results and reactors emit warnings only. Rationale: Reduce complexity and avoid unused prompts. Date/Author: 2025-12-17 / Assistant.

## Outcomes & Retrospective

All phases completed: engine exposes pure `StateUpdates` with I/O-free tests; runtime crate owns watcher/reactor loop and emits `RuntimeEvent`s; CLI watch handler now ~100 lines focused on runtime wiring and `TraceView` observation. Intervention support was added and later removed as unused, simplifying runtime events and eliminating executor prompts.
...
2. **Create `agtrace-runtime` crate**: Register the new crate in the workspace and depend on `agtrace-engine` and `agtrace-types`. Move `SessionState`, `Reaction`, `Reactor` trait, and `run_reactors`-style coordination here. Relocate `streaming/watcher.rs` logic (filesystem polling and `SessionUpdate`) and create a `Runtime` struct that owns a worker thread consuming `Receiver<StreamEvent>` and yielding a typed `RuntimeEvent` iterator/stream. Define `RuntimeEvent` and `RuntimeConfig` per the user contract, keeping runtime free of CLI/OS specifics. Ensure runtime emits `StateUpdated`, `ReactionTriggered`, `SessionRotated`, `SessionAttached`, `Waiting`, and `FatalError` events without direct UI calls.
3. **Wire interventions**: (retired) A prior iteration added executor wiring; this has been removed. Reactors now emit warnings only.
4. **Observer-driven UI**: Remove `TuiRenderer` from the reactor set; instead, adapt it (or a new renderer) to consume `RuntimeEvent` alongside the existing `TraceView` wiring. Adjust `watch` handler to build `RuntimeConfig` (provider, reactors, paths, poll interval), start the runtime, and subscribe to its events, forwarding updates to `TraceView` methods. Keep handler code around 100 LOC by delegating state updates and reactions to runtime.
5. **Migration and cleanup**: Update imports and module wiring so CLI relies on runtime APIs instead of direct engine parsing or watcher internals. Ensure dependency direction matches the required chain; adjust `agtrace-cli` to no longer access moved types directly except via runtime exports. Add targeted tests in `agtrace-runtime` for event propagation and reaction handling, and update CLI tests to cover runtime subscription hooks.
...
- (Retired) Intervention executor wiring was removed; runtime no longer defines Intervention types or executor traits.
- Update `agtrace-cli` to construct `RuntimeConfig` and refactor `handlers/watch.rs` to subscribe to runtime events and render via `TraceView`, with `TuiRenderer` moved to a pure observer.
- Run `cargo test -p agtrace-engine`, `cargo test -p agtrace-runtime` (new), and `cargo test -p agtrace-cli`; add focused unit tests for runtime event emissions if feasible.

## Validation and Acceptance

Acceptance requires: engine tests demonstrating `extract_state_updates` correctly updates session state from JSON metadata and token usage snapshots without I/O; `agtrace-runtime` tests showing `RuntimeEvent` emission for updates and reactions; `agtrace-cli` `watch` handler simplified to ~100 LOC that primarily wires runtime config and subscribes to events; and manual or test verification that reactions surface as warnings without prompting for executor actions. `cargo test` commands listed above must pass.
...
- `agtrace-runtime`: `pub struct SessionState { ... }`, `pub enum Reaction { Continue, Warn(String) }`, `pub trait Reactor { fn name(&self) -> &str; fn handle(&mut self, ctx: ReactorContext) -> anyhow::Result<Reaction>; }`, `pub enum RuntimeEvent { SessionAttached { display_name: String }, StateUpdated { state: Box<SessionState>, new_events: Vec<AgentEvent> }, ReactionTriggered { reactor_name: String, reaction: Reaction }, SessionRotated { old_path: PathBuf, new_path: PathBuf }, Waiting { message: String }, FatalError(String) }`, `pub struct RuntimeConfig { provider: Box<dyn LogProvider>, reactors: Vec<Box<dyn Reactor>>, watch_path: PathBuf, poll_interval: Duration, explicit_target: Option<String>, project_root: Option<PathBuf> }`, and `pub struct Runtime` that consumes `StreamEvent` and yields `RuntimeEvent`.
- `agtrace-cli`: No intervention executor; `watch` builds `RuntimeConfig` without executor and renders warnings via `TraceView`.
