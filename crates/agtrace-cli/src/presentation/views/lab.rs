    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for (i, event) in self.data.matches.iter().enumerate() {
            ...
            if self.data.json_output {
                ...
                // Simple text format
                match &event.payload {
                    crate::presentation::view_models::EventPayloadViewModel::User { text } => {
                        writeln!(f, "User: {}", text)?;
                    }
                    crate::presentation::view_models::EventPayloadViewModel::Reasoning { text } => {
                        writeln!(f, "Reasoning: {}", text)?;
                    }
                    crate::presentation::view_models::EventPayloadViewModel::ToolCall {
                        name,
                        arguments,
                    } => {
                    ...
                    crate::presentation::view_models::EventPayloadViewModel::ToolResult {
                        output,
                        is_error,
                    } => {
                        writeln!(f, "ToolResult (error={}): {}", is_error, output)?;
                    }
                    crate::presentation::view_models::EventPayloadViewModel::Message { text } => {
                        writeln!(f, "Message: {}", text)?;
                    }
                    crate::presentation::view_models::EventPayloadViewModel::TokenUsage {
                        input,
                        output,
                        total,
                        cache_creation,
                    ...
                    crate::presentation::view_models::EventPayloadViewModel::Notification {
                        text,
                        level,
                    } => {
