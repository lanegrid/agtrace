...
impl<'a> WatchEventView<'a> {
    ...
    fn render_verbose(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if matches!(self.mode, ViewMode::Verbose) {
            if let WatchEventViewModel::StreamUpdate { state, .. } = self.event {
                writeln!(f, "\n{}", "Debug Info:".dimmed())?;
    ...
    fn render_stream_update(
        ...
        events: &[crate::presentation::v2::view_models::EventViewModel],
        _turns: Option<&[crate::presentation::v2::view_models::TurnUsageViewModel]>,
    ) -> fmt::Result {
