...
const MODEL_SPECS: &[ModelSpec] = &[
    ...
    ModelSpec::new("gpt-5.2", 400_000),
    // GPT-5.1 series
    ModelSpec::new("gpt-5.1-codex-max", 400_000),
    ModelSpec::new("gpt-5.1-codex-mini", 400_000),
    ModelSpec::new("gpt-5.1-codex", 400_000),
    ModelSpec::new("gpt-5.1", 400_000),
    // GPT-5 series
    ModelSpec::new("gpt-5-codex-mini", 400_000),
    ModelSpec::new("gpt-5-codex", 400_000),
    ModelSpec::new("gpt-5", 400_000),
];
...
mod tests {
    ...
    fn test_model_limits_coverage() {
        ...
        assert_eq!(limits.get("gpt-5.2"), Some(&400_000));

        // Verify GPT-5.1 series
        assert_eq!(limits.get("gpt-5.1-codex-max"), Some(&400_000));
        assert_eq!(limits.get("gpt-5.1-codex-mini"), Some(&400_000));
        assert_eq!(limits.get("gpt-5.1-codex"), Some(&400_000));
        assert_eq!(limits.get("gpt-5.1"), Some(&400_000));

        // Verify GPT-5 series
        assert_eq!(limits.get("gpt-5-codex-mini"), Some(&400_000));
        assert_eq!(limits.get("gpt-5-codex"), Some(&400_000));
        assert_eq!(limits.get("gpt-5"), Some(&400_000));
    }
