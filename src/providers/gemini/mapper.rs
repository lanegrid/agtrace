...
pub fn normalize_gemini_session(session: &Value) -> Vec<AgentEventV1> {
    ...
    let project_hash = session.get("projectHash").and_then(|v| v.as_str()).unwrap_or("unknown").to_string();
    let session_id = session.get("sessionId").and_then(|v| v.as_str()).map(|s| s.to_string());
    ...
    if let Some(msgs) = session.get("messages").and_then(|v| v.as_array()) {
        for msg in msgs {
            ...
            match msg_type {
                "user" => {
                    let mut ev = AgentEventV1::new(Source::Gemini, project_hash.clone(), ts.clone(), EventType::UserMessage);
                    ...
                    ev.text = msg.get("content").and_then(|v| v.as_str()).map(|s| s.to_string());
                    last_user_event_id = ev.event_id.clone();
                    ev.raw = msg.clone();
                    events.push(ev);
                }
                "gemini" => {
                    let mut ev = AgentEventV1::new(Source::Gemini, project_hash.clone(), ts.clone(), EventType::AssistantMessage);
                    ev.session_id = session_id.clone();
                    ev.event_id = msg_id.clone();
                    ...
                    if let Some(thoughts) = msg.get("thoughts").and_then(|v| v.as_array()) {
                        for (idx, thought) in thoughts.iter().enumerate() {
                            let mut rev = AgentEventV1::new(Source::Gemini, project_hash.clone(), ts.clone(), EventType::Reasoning);
                            ...
                            let subject = thought.get("subject").and_then(|v| v.as_str()).unwrap_or("");
                            let description = thought.get("description").and_then(|v| v.as_str()).unwrap_or("");
                    ...
                    if let Some(tool_calls) = msg.get("toolCalls").and_then(|v| v.as_array()) {
                        for tool_call in tool_calls {
                            let mut tev = AgentEventV1::new(Source::Gemini, project_hash.clone(), ts.clone(), EventType::ToolCall);
                            tev.session_id = session_id.clone();
                            tev.event_id = tool_call.get("id").and_then(|v| v.as_str()).map(|s| s.to_string());
                            ...
                            tev.tool_name = tool_call.get("name").and_then(|v| v.as_str()).map(|s| s.to_string());
                            ...
                            let result_display = tool_call.get("resultDisplay").and_then(|v| v.as_str()).unwrap_or("");
                ...
                "info" => {
                    let mut ev = AgentEventV1::new(Source::Gemini, project_hash.clone(), ts.clone(), EventType::SystemMessage);
                    ev.session_id = session_id.clone();
                    ev.event_id = msg_id.clone();
                    ev.parent_event_id = last_user_event_id.clone();
