...
fn test_parse_claude_simple_session() {
    ...
    assert!(
        events.len() >= 4,
        "Expected at least 4 events (user, thinking, tool_call, assistant)"
    );
    ...
    assert!(
        reasoning_event.parent_event_id.is_some(),
        "Reasoning should have parent_event_id"
    );
    assert_eq!(
        reasoning_event.parent_event_id, user_event.event_id,
        "Reasoning parent should be user message"
    );
